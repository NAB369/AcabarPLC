...
export default function LoanRepaymentDetailPage() {
  ...
  const handleExportXLS = () => {
    ...
    const headers = ['No', 'Due Date', 'Principal', 'Interest', 'Col Fee', 'Total Due', 'Balance', 'Penalty Payoff', 'Status'];
    let runningBalance = loan.principalAmount || 0;
    const rows = loan.repaymentSchedules.map((s: any) => {
      ...
      return [
        ...
        Number(s.principalComponent || 0).toFixed(2),
        Number(s.interestComponent || 0).toFixed(2),
        '0.00',
        totalDue.toFixed(2),
    ...
    <div style={{ animation: 'fadeIn 0.6s ease-out' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
          <button onClick={() => router.back()} style={{ border: '1px solid var(--border-color)', background: 'white', padding: '0.6rem', borderRadius: '12px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <ArrowLeft size={20} />
          </button>
          <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 350px', gap: '2rem' }}>
              ...
              <tbody>
                {schedulesWithBalance.map((s: any) => (
                  <tr key={s.id} style={{ opacity: s.status === 'PAID' ? 0.7 : 1 }}>
                    ...
                    <td style={{ color: Number(s.penaltyAmount) > 0 ? '#ef4444' : 'var(--text-muted)', fontWeight: Number(s.penaltyAmount) > 0 ? '700' : '400' }}>
                      ${Number(s.penaltyAmount || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </td>
                    <td>
                      {s.status === 'PAID' ? (
                        <span style={{ color: '#16a34a', display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.75rem', fontWeight: '700' }}>
                        ...
                        <span style={{ color: '#dc2626', display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.75rem', fontWeight: '700' }}>
                        ...
                        <span style={{ color: '#d97706', display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.75rem', fontWeight: '700' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <div className="card" style={{ padding: '1.5rem' }}>
            ...
            <div style={{ position: 'relative', height: '8px', backgroundColor: '#f1f5f9', borderRadius: '4px', overflow: 'hidden', marginBottom: '1rem' }}>
      ...
      {showPayModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', animation: 'fadeIn 0.2s' }}>
          <div className="card" style={{ width: '400px', padding: '2rem', animation: 'slideUp 0.3s' }}>
            ...
            <div style={{ backgroundColor: '#f8fafc', padding: '1.5rem', borderRadius: '16px', textAlign: 'center', marginBottom: '1.5rem' }}>
              ...
              {Number(selectedInstallment.penaltyAmount) > 0 && (
                <div style={{ fontSize: '0.875rem', color: '#ef4444', fontWeight: '600', marginTop: '0.5rem' }}>Includes ${Number(selectedInstallment.penaltyAmount).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} Late Penalty</div>
              )}
            </div>
              ...
              {paymentMethod === 'CASH' && (
                <div style={{ marginTop: '0.5rem', animation: 'fadeIn 0.3s ease-out' }}>
                  ...
                  <input 
                    ...
                    style={{ width: '100%', padding: '0.75rem', border: '1px solid var(--border-color)', borderRadius: '8px', fontSize: '0.875rem', backgroundColor: '#f8fafc' }}
              ...
              {(paymentMethod === 'KHQR' || paymentMethod === 'TRANSFER') && (
                <div style={{ marginTop: '0.5rem', animation: 'fadeIn 0.3s ease-out' }}>
                  ...
                  <input 
                    ...
                    style={{ width: '100%', padding: '0.5rem', border: '1px dashed var(--border-color)', borderRadius: '8px', fontSize: '0.875rem', backgroundColor: '#f8fafc', cursor: 'pointer' }}
                    accept="image/*,.pdf"
                  />
                  {paymentProof && paymentProof.type.startsWith('image/') && (
                    <div style={{ marginTop: '0.75rem', borderRadius: '8px', overflow: 'hidden', border: '1px solid var(--border-color)', display: 'flex', justifyContent: 'center', backgroundColor: '#f1f5f9' }}>
      ...
      <div className={`print-overlay ${showPreview ? 'preview-active' : 'preview-hidden'}`} style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 1000, alignItems: 'center', justifyContent: 'center', animation: 'fadeIn 0.2s' }}>
          <div className="card print-statement" style={{ width: '800px', maxWidth: '90vw', maxHeight: '90vh', overflowY: 'auto', padding: '2.5rem', animation: 'slideUp 0.3s', backgroundColor: 'white' }}>
            ...
            <div style={{ position: 'relative', marginBottom: '2.5rem' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ width: '120px' }}>
                  <img src="/acabar-logo.png" alt="Logo" style={{ width: '90px', height: '90px', objectFit: 'contain' }} />
                </div>
                <div style={{ flex: 1, textAlign: 'center', color: '#000' }}>
                  <div style={{ fontFamily: '"Khmer OS Muol Light", "Khmer OS Battambang", serif', fontSize: '1.35rem', fontWeight: 'bold', marginBottom: '0.25rem' }}>អាខាបារ ម.ក</div>
                  <div style={{ fontSize: '1.25rem', fontWeight: 'bold', marginBottom: '0.25rem', letterSpacing: '0.05em' }}>ACABAR .PLC</div>
                  <div style={{ fontFamily: '"Khmer OS Muol Light", "Khmer OS Battambang", serif', fontSize: '1.35rem', fontWeight: 'bold' }}>តារាងកាលវិភាគសងប្រាក់</div>
                </div>
                <div style={{ width: '120px', display: 'flex', justifyContent: 'flex-end' }}>
                  {khqrImage && <img src={khqrImage} alt="KHQR" style={{ width: '90px', height: '90px', objectFit: 'contain' }} />}
                </div>
              </div>
            </div>
              ...
              <thead>
                <tr style={{ backgroundColor: '#fff' }}>
                  <th style={{ border: '1px solid #000', padding: '0.25rem', textAlign: 'center', fontSize: '0.85rem' }}>
                    <div style={{ color: '#1d4ed8' }}>លេខ</div>
                    <div>No</div>
                  </th>
                  <th style={{ border: '1px solid #000', padding: '0.25rem', textAlign: 'center', fontSize: '0.85rem' }}>
                    <div style={{ color: '#1d4ed8' }}>ថ្ងៃបង់ប្រាក់</div>
                    <div>Repayment Date</div>
                  </th>
                  <th style={{ border: '1px solid #000', padding: '0.25rem', textAlign: 'center', fontSize: '0.85rem' }}>
                    <div style={{ color: '#1d4ed8' }}>ប្រាក់ដើម</div>
                    <div>Principle</div>
                  </th>
                  <th style={{ border: '1px solid #000', padding: '0.25rem', textAlign: 'center', fontSize: '0.85rem' }}>
                    <div style={{ color: '#1d4ed8' }}>ការប្រាក់</div>
                    <div>Interest</div>
                  </th>
                  <th style={{ border: '1px solid #000', padding: '0.25rem', textAlign: 'center', fontSize: '0.85rem' }}>
                    <div style={{ color: '#1d4ed8' }}>សេវាប្រមូល</div>
                    <div>Col Fee</div>
                  </th>
                  <th style={{ border: '1px solid #000', padding: '0.25rem', textAlign: 'center', fontSize: '0.85rem' }}>
                    <div style={{ color: '#1d4ed8' }}>សរុប</div>
                    <div>Total</div>
                  </th>
                  <th style={{ border: '1px solid #000', padding: '0.25rem', textAlign: 'center', fontSize: '0.85rem' }}>
                    <div style={{ color: '#1d4ed8' }}>ប្រាក់ដើមនៅសល់</div>
                    <div>Balance</div>
                  </th>
                  <th style={{ border: '1px solid #000', padding: '0.25rem', textAlign: 'center', fontSize: '0.85rem' }}>
                    <div style={{ color: '#1d4ed8' }}>ប្រាក់ផាកពិន័យបង់យឺត</div>
                ...
                {schedulesWithBalance.map((s: any) => {
                  ...
                  return (
                    <tr key={s.id}>
                      ...
                      <td style={{ border: '1px solid #000', padding: '0.25rem 0.5rem', textAlign: 'right', fontSize: '0.85rem', color: Number(s.penaltyAmount) > 0 ? '#dc2626' : '#000' }}>{Number(s.penaltyAmount || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                    </tr>
                  );
                })}
                {/* Total Row */}
            ...
            <div className="no-print" style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '2rem' }}>
              <div style={{ position: 'relative' }}>
                ...
                {showExportMenu && (
                  <div style={{ position: 'absolute', bottom: '100%', right: 0, marginBottom: '0.5rem', backgroundColor: 'white', border: '1px solid var(--border-color)', borderRadius: '8px', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)', overflow: 'hidden', minWidth: '160px', zIndex: 10 }}>
                    <button onClick={() => { setShowExportMenu(false); handleExportXLS(); }} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', width: '100%', padding: '0.75rem 1rem', background: 'none', border: 'none', borderBottom: '1px solid var(--border-color)', cursor: 'pointer', textAlign: 'left', fontSize: '0.875rem', color: '#333' }}>
                      <Download size={16} /> Export as XLS
                    </button>
                    <button onClick={() => { setShowExportMenu(false); window.print(); }} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', width: '100%', padding: '0.75rem 1rem', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left', fontSize: '0.875rem', color: '#333' }}>
      ...
      {showCropModal && tempImageSrc && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.8)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', animation: 'fadeIn 0.2s' }}>
          <div className="card" style={{ width: '800px', maxWidth: '90vw', padding: '2rem', backgroundColor: 'white', display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
            ...
            <div style={{ position: 'relative', width: '100%', maxHeight: '60vh', overflow: 'auto', backgroundColor: '#f1f5f9', borderRadius: '8px', display: 'flex', justifyContent: 'center' }}>
              <ReactCrop 
                crop={crop} 
